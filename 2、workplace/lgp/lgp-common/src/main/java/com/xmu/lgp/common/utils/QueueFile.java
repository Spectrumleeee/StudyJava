...
public class QueueFile {
    ...
    /**
     * Constructs a new queue backed by the given file. Only one
     * {@code QueueFile} instance should access a given file at a time.
     */
    ...
    private int wrapPosition(int position) {
        return position < fileLength ? position : HEADER_LENGTH + position
                - fileLength;
    }
    ...
    public synchronized void add(byte[] data, int offset, int count)
        ...
        int position = wasEmpty ? HEADER_LENGTH : wrapPosition(last.position
        ...
        writeHeader(fileLength, elementCount + 1, firstPosition,
                newLast.position);
        last = newLast;
        elementCount++;
        if (wasEmpty)
    ...
    private void expandIfNecessary(int dataLength) throws IOException {
        ...
        if (last.position < first.position) {
            ...
            writeHeader(newLength, elementCount, first.position,
                    newLastPosition);
    ...
    /**
     * Invokes the given reader once for each element in the queue, from eldest
     ...
     */
    class ElementInputStream extends InputStream {
    ...
    interface ElementReader {
         ...
         * 
         * @param in
         *            stream of element data. Reads as many bytes as requested,
         *            unless fewer than the request number of bytes remains, in
         *            which case it reads all the remaining bytes.
         * @param length
