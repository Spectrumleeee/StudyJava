...
public class ProxyThread implements Callable<Object>, Constants, Closeable {
    ...
    public Object call() throws Exception {
        ...
        while (socket.isConnected()) {
            ProxyUtils.readHeaders(this.output, this.headerLines,
                    this.requestInput, HeaderEncoding);
            parseRequestHeaders();
            logger.debug("received request %s %s", uriToks[1], uriToks[3]);
            if (Debug) {
                parent.reply(new HttpResponse(StatusCode.Forbidden, TestHtml
                        .getBytes(HeaderEncoding), TextHtml));
                this.skipBody(this.requestHeaders, this.requestInput);
                continue;
            }
            if ("https://".equalsIgnoreCase(uriToks[0].toString())) {
                parent.reply(new HttpResponse(StatusCode.MethodNotAllowed,
                        "https not supported".getBytes(HeaderEncoding),
                        TextHtml));
                this.skipBody(this.requestHeaders, this.requestInput);
                continue;
            }
            ...
            try {
                ...
                parent.reply(new HttpResponse(StatusCode.ServiceUnavailable,
                        buffer.toString().getBytes(HeaderEncoding), TextHtml));
                continue;
            }
            logger.debug("received response %s %s %s", uriToks[1], uriToks[3],
                    responseFirstLine[1]);
            this.mockBody = null;
    ...
    boolean parseRequestHeaders() throws IOException {
        ...
        if (index1 > 0 && index2 > index1) {
            ...
            if (b.startsWith("/")) {
                String host = ProxyUtils.getHeaderValue(this.requestHeaders,
                        Host);
            ...
            uriToks[1] = host;
            uriToks[2] = port;
            String s1 = ProxyUtils.getHeaderValue(this.requestHeaders,
                    Connection);
            String s2 = ProxyUtils.getHeaderValue(this.requestHeaders,
                    ProxyConnection);
            keep = "keep-alive".equalsIgnoreCase(s1)
                    || "keep-alive".equalsIgnoreCase(s2);
    ...
    protected void skipBody(List<NameValuePair> requestHeaders,
            InputStream input) throws IOException {
        String _length = ProxyUtils.getHeaderValue(requestHeaders,
                ContentLength);
    ...
    protected void transferBody(OutputStream output) throws IOException {
        String _length = ProxyUtils.getHeaderValue(this.requestHeaders,
                ContentLength);
        if (_length != null && _length.length() > 0) {
            long length = Long.parseLong(_length);
    ...
    protected void sendRequest() throws IOException {
        ...
        try {
            ...
            socket = new Socket(HttpProxyClient.PROXY_SERVER_HOST,
                    Integer.parseInt(HttpProxyClient.PROXY_SERVER_PORT));
            ...
            responseInput = new DataInputStream(new BufferedInputStream(
                    socket.getInputStream()));
            while (true) {
                ProxyUtils.readHeaders(this.output, this.headerLines,
                        responseInput, encoding);
    ...
    public void sendRequestHeaders(OutputStream output, String encoding)
            throws IOException {
    ...
    public void sendResponseHeaders(OutputStream output, String encoding)
            throws IOException {
    ...
    protected void sendResponse() throws IOException {
        HttpResponse response = new HttpResponse(
                Integer.parseInt(responseFirstLine[1]), responseFirstLine[2]);
        response.setVersionTok(responseFirstLine[0]);
        if (keep) {
            ProxyUtils
                    .setHeader(this.responseHeaders, Connection, "keep-alive");
        }
