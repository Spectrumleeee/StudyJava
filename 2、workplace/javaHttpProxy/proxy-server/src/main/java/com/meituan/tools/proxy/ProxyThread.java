...
public class ProxyThread implements Callable<Object>, Constants, Closeable {
    ...
    public Object call() throws Exception {
        ...
        while (socket.isConnected()) {
            ...
            parseRequestHeaders();
            logger.debug("received request %s %s %s", uriToks[1], uriToks[3], Thread.currentThread().getName());
            if (Debug) {
            ...
            logger.debug("received response %s %s %s %s", uriToks[1], uriToks[3], responseFirstLine[1], Thread.currentThread().getName());
            this.mockBody = null;
...
//            System.out.println("Sent to Host: " + uriToks[1].toString());
//            System.out.println("Sent to Port: " + ((Number) uriToks[2]).intValue());
