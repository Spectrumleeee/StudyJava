...
public class HttpProxyServer implements Closeable {
    ...
    public HttpProxyServer(String host, int port, RequestInterceptor interceptor)
            throws IOException {
    ...
    public static void main(String[] args) throws Exception {
        ...
        RequestInterceptor interceptor = new ComboInterceptor(
                new GZIPInterceptor(), new LogRequestInterceptor(new File(
                        args[2])));
        try (HttpProxyServer proxy = new HttpProxyServer(args[0],
            ...
            if (args.length > 3) {
                ...
                System.out.println("Proxy server started in port " + args[1]);
